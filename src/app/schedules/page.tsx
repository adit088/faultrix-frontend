...
// Mock active windows: business hours mon-fri + evening chaos
function isActive(day: number, hour: number) {
  ...
  return { active: biz || eve, peak: peak }
}
...
export default function SchedulesPage() {
  return (
    <div className="space-y-6 animate-slide-up">
      ...
      {/* Heatmap */}
      <div className="bg-[#111118] rounded-xl border border-[#1e1e2e] p-5 overflow-x-auto">
        <h3 className="text-sm font-semibold text-[#e8e8f0] mb-4">Weekly Chaos Activity Heatmap</h3>
        <div className="flex gap-2">
          {/* Hour labels */}
          <div className="flex flex-col gap-1 pt-6">
            {hours.filter((_, i) => i % 3 === 0).map(h => (
              <div key={h} className="text-[9px] font-mono text-[#4a4a6a] h-4 flex items-center pr-2" style={{ height: "calc((100% - 24px) / 8)" }}>
                {h}
              </div>
            ))}
          </div>
              ...
              <div className="flex flex-col gap-0.5">
                {hours.map((_, hi) => {
                  ...
                  return (
                    <div
                      key={hi}
                      className="h-3 rounded-sm transition-all hover:scale-110 cursor-pointer"
                      style={{
                        background: peak
                          ? "rgba(255, 59, 92, 0.7)"
                          : active
