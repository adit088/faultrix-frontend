  ...
  return (
    <div className="space-y-6 animate-slide-up">
      ...
      <div className="flex items-center justify-between">
        ...
        <button className="px-4 py-2 rounded-lg bg-[#6c47ff] text-white text-sm font-medium hover:bg-[#7c57ff] transition-colors">
          + New Rule
        </button>
      </div>
      ...
      <div className="bg-[#111118] rounded-xl border border-[#1e1e2e] overflow-hidden">
        ...
        {rules.map(rule => (
          <div key={rule.id} className={cn(
            "grid grid-cols-12 gap-3 px-5 py-4 border-b border-[#1e1e2e]/50 items-center hover:bg-[#1a1a28]/50 transition-colors",
            ...
            <div className="col-span-2">
              <span className={cn("text-[10px] font-mono px-2 py-0.5 rounded border", modeColors[rule.targetingMode ?? "EXACT"])}>
                {rule.targetingMode ?? "EXACT"}
              </span>
            </div>
            <div className="col-span-2">
              <div className="flex items-center gap-2">
                <div className="flex-1 h-1 bg-[#1e1e2e] rounded-full">
                  <div className="h-full bg-[#ff3b5c] rounded-full" style={{ width: pct(rule.failureRate) }} />
                </div>
                <span className="text-xs font-mono text-[#e8e8f0] w-8">{pct(rule.failureRate)}</span>
              </div>
            </div>
            <div className="col-span-2">
              <span className="text-xs font-mono text-[#f59e0b]">{fmtMs(rule.maxDelayMs)}</span>
            </div>
            <div className="col-span-2">
              <div className="flex items-center gap-2">
                <div className="flex-1 h-1 bg-[#1e1e2e] rounded-full">
                  <div
                    ...
                    style={{
                      width: pct(rule.blastRadius ?? 0),
                      background: (rule.blastRadius ?? 0) > 0.7 ? "#ff3b5c" : (rule.blastRadius ?? 0) > 0.4 ? "#f59e0b" : "#00e5a0"
                    }}
                  />
                </div>
            ...
            <div className="col-span-1 flex justify-end">
              <button
                ...
                className={cn(
                  ...
                  rule.enabled ? "bg-[#00e5a0]/80" : "bg-[#2a2a3e]"
                )}
              >
                <div className={cn(
