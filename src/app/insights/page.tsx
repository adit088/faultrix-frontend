...
const insights = [
  {
    ...
    confidence: 0.92,
    desc: "High failure rate on user-service is propagating to downstream payment-service. Blast radius exceeds safe threshold.",
    fix: `// Recommended: Add circuit breaker\n@CircuitBreaker(name = "payment", fallbackMethod = "paymentFallback")\npublic PaymentResponse process(PaymentRequest req) { ... }`,
    algorithm: "Causal Impact Analysis",
  },
  {
    level: "WARNING",
    title: "Latency Spike Pattern",
    target: "inventory-service",
    confidence: 0.78,
    desc: "Inventory service shows periodic latency spikes every ~45 minutes. Likely correlated with scheduled jobs.",
    fix: `# Schedule chaos tests outside job windows\ncron: "0 */1 * * *"  # ← adjust to avoid :00 and :45`,
    algorithm: "Time-Series Anomaly Detection",
  },
  {
    level: "INFO",
    title: "Blast Radius Recommendation",
    target: "All rules",
    confidence: 0.65,
    desc: "Current average blast radius of 42% is above recommended 25% for initial chaos experiments.",
    fix: `// Reduce blast radius gradually\nrule.setBlastRadius(0.25); // Start low, increase over sprints`,
    algorithm: "Risk Scoring Engine",
  },
  {
    level: "WARNING",
    title: "Uncovered Service Paths",
    target: "auth-service, notification-service",
    confidence: 0.71,
    desc: "No chaos rules configured for auth or notification services. These represent untested resilience gaps.",
    fix: `// Add rules for uncovered services\nPOST /api/v1/chaos/rules\n{ "target": "auth-service", "failureRate": 0.1 }`,
    algorithm: "Coverage Gap Detector",
  },
]
    ...
    <div className="space-y-6 animate-slide-up">
      ...
      <div className="grid grid-cols-2 gap-4">
        {insights.map((ins, i) => {
          ...
          return (
            <div key={i} className={cn("bg-[#111118] rounded-xl border p-5 space-y-3", colors.border)}>
              <div className="flex items-start justify-between gap-2">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-[#4a4a6a] font-mono mt-0.5">{ins.target}</p>
                </div>
              ...
              <div className="h-1 bg-[#1e1e2e] rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${ins.confidence * 100}%`,
                    background: ins.level === "CRITICAL" ? "#ff3b5c" : ins.level === "WARNING" ? "#f59e0b" : "#6c47ff"
                  }}
                />
              </div>
