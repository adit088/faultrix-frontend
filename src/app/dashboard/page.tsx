  ...
  return (
    <div className="space-y-6 animate-slide-up">
      ...
      <div className="grid grid-cols-4 gap-4">
        {stats.map(s => (
          <div key={s.label} className="stat-card relative bg-[#111118] rounded-xl border border-[#1e1e2e] p-5">
            <p className="text-[11px] font-mono uppercase tracking-widest text-[#4a4a6a]">{s.label}</p>
            <p className="text-3xl font-bold mt-1 font-mono" style={{ color: s.color }}>{s.value}</p>
      ...
      <div className="grid grid-cols-3 gap-4">
        {/* Chart — 2 cols */}
        <div className="col-span-2 bg-[#111118] rounded-xl border border-[#1e1e2e] p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-3 text-xs text-[#8888aa]">
              ...
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-[#2a2a3e] inline-block"/>Skipped</span>
            </div>
          </div>
        ...
        {/* Blast radius */}
        <div className="bg-[#111118] rounded-xl border border-[#1e1e2e] p-5 flex flex-col items-center justify-center">
          ...
          {mockAnalytics.topTargets.map((t, i) => {
            ...
            return (
              <div key={t.target} className="flex items-center gap-3">
                ...
                <span className="text-sm text-[#e8e8f0] w-40 truncate">{t.target}</span>
                <div className="flex-1 h-1.5 bg-[#1e1e2e] rounded-full overflow-hidden">
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-[#6c47ff] to-[#a78bfa]"
                    style={{ width: `${pctVal}%` }}
                  />
                </div>
                <span className="text-xs font-mono text-[#8888aa] w-12 text-right">{fmt(t.injectionCount)}</span>
              </div>
            )
          })}
        </div>
